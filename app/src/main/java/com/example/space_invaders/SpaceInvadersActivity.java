...
import static androidx.core.content.ContextCompat.startActivity;import android.content.Context;
import android.content.Intent;
import android.content.SharedPreferences; ///
...
public class SpaceInvadersActivity extends Activity {
    ...
    public class SpaceInvadersView extends SurfaceView implements Runnable {
        ...
        boolean mode = prefs.getBoolean("spaceInvadersGameMode", true);
...
// This special constructor method runs
    ...
    private void showStartDialog(boolean firstStart){
        ...
        new AlertDialog.Builder(this).setTitle("Inwazja!").
                setMessage("Armia kosmitów atakuje Twój statek!\n" +
    ...
    private void showStartDialogForHardcoreMode(boolean firstStart){

        new AlertDialog.Builder(this).setTitle("Witaj w trybie hardcore!").
                setMessage("W tym trybie musisz oprzeć się inwazji kosmitów, jednak nie będzie to już takie łatwe!\n\n" +
                        "Po wciśnięciu OK, kliknij gdziekolwiek, aby rozpocząć grę.\n" +
                        "Zasady pozostaną niezmienione z wyjątkiem tego, że...\n\n" +
                        "Nię będziesz już widział nut literowych!- zamiast tego będziesz musiał rozpoznać daną nutę ze słuchu\n" +
                        "i na podstawie usłyszanego dźwięku zestrzelić odpowiedniego kosmitę.\n" +
                        "poza tym nuty w tym trybie gry odgrywane będą w kolejności całkowicie losowej.\n\n" +
                        "Postaraj się, bo życie załogi statku jest teraz tylko w Twoich rękach!").
                setPositiveButton("ok", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        dialog.dismiss();
        ...
        prefs = getSharedPreferences("SaveData", MODE_PRIVATE);
        SharedPreferences.Editor editor = prefs.edit();
        editor.putBoolean("firstStart hardcore mode", false);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.GINGERBREAD) {
            editor.apply();
        }

        if(!firstStart){
            editor.putBoolean("secondStart hardcore mode", false);
        }
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        boolean firstStart2 = prefs.getBoolean("firstStart hardcore mode", true);
        boolean secondStart2 = prefs.getBoolean("secondStart hardcore mode", true);
        ...
        if(mode) {
            ...
            if (secondStart2) {
                showStartDialogForHardcoreMode(firstStart2);
            }
        }
